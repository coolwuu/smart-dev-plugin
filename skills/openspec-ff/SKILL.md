...
4. **Create artifacts in sequence until apply-ready**
   ...
   a. **For each artifact that is `ready` (dependencies satisfied)**:
      ...
      - If the `openspec-clarify` skill is available, quickly scan the artifact for critical ambiguities (TBD, contradictions, requirements without scenarios). If found, invoke it before continuing. Otherwise skip to maintain momentum.
...
- If the `openspec-analyze` skill is available, invoke it to check artifact consistency before recommending implementation. If not available, skip this step.
- Summarize:
